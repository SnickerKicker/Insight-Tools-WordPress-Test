/**
 *  Advanced iframe pro external workaround file vPARAM_VERSION
...
function aiInitElementResize_PARAM_ID() {
   aiDebugExtended('aiInitElementResize_PARAM_ID');
...
function aiExecuteWorkaround_PARAM_ID(init) {
    aiDebugExtended('aiExecuteWorkaround_PARAM_ID');
	...
	// we only resize if we have a change of more than 5 pixel or we are in the init phase
        if (iframe_PARAM_ID_last_height === -1 || Math.abs(iframe_PARAM_ID_last_height - newHeight) >= 5) { 
		 ...
		 var data = { 'aitype' : 'height',
		      ...
		      'anchor' : anchorPosition,
               ...
               showItNow = false
               setTimeout(function () { aiResizeLater_PARAM_ID(); }, onload_resize_delay);
            }
        ...
        if (enable_responsive_iframe === 'true' && init) {
            ...
            window.setTimeout(aiInitResize_PARAM_ID, onload_resize_delay + 1000);
        }
        if (showItNow) {
...
function aiResizeLater_PARAM_ID() {
    aiDebugExtended('aiResizeLater_PARAM_ID');
   ...
   if (newHeight > 10) { // Only resize if the height is > 10
      if (Math.abs(iframe_PARAM_ID_last_height - newHeight) >= 5) { // we only resize if we have a change of more than 5 pixel
...
function aiInitResize_PARAM_ID() {
  aiDebugExtended('aiInitResize_PARAM_ID');
...
jQuery(window).resize(function() {
    if (enable_responsive_iframe === 'true') {
      ...
      if (iframeWidth !== newIframeWidth) {
          ...
          // set overflow to visible again.
          if (keepOverflowHidden === 'false') {
              window.setTimeout(aiResizeLater_PARAM_ID,500);
          }
      }
    }
});
...
var doIt = aiLoadExternalConfig(domain_PARAM_ID);
...
if (doIt) {
  ...
  if (typeof change_iframe_links_href === 'undefined') {
      var change_iframe_links_href = 'PARAM_CHANGE_IFRAME_LINKS_HREF';
  }
  ...
  if (typeof use_iframe_title_for_parent === 'undefined') {
      var use_iframe_title_for_parent = 'PARAM_USE_IFRAME_TITLE_FOR_PARENT';
  }
  ...
  if (typeof scroll_to_top === 'undefined') {
      var scroll_to_top = 'PARAM_SCROLL_TO_TOP';
  }
  ...
  if (typeof disable_right_click === 'undefined') {
      var disable_right_click = 'PARAM_SRC_HIDE' === '' ? 'false' : 'true';
  }
  ...
  aiAddOnloadEvent(aiExecuteWorkaround_PARAM_ID);
