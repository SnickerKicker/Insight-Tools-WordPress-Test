...
class DUP_PRO_Web_Services extends AbstractAjaxService
{
    ...
    function duplicator_pro_ftp_send_file_test()
    {
        ...
        if (!$password) {
            $statusMsgsObj->addMessage(__('Error: You haven\'t specified password', 'duplicator-pro'));
        }

        if (!$storage_folder || !$server || !$port || $port < 0 || !$username || !$password) {
            $isValid = false;
        }
    ...
    function duplicator_pro_sftp_send_file_test()
    {
        ...
        if (!$private_key && !$password) {
            $statusMsgsObj->addMessage(__('ERROR: You must specify either private_key or password', 'duplicator-pro'));
        }
        if ($private_key && !$private_key_password) {
            $statusMsgsObj->addMessage(__('ERROR: You must specify private key password along with private key', 'duplicator-pro'));
        }
        ...
        try {
            ...
            $sftp          = $dup_phpseclib->connect_sftp_server($server, $port, $username, $password, $private_key, $private_key_password, $statusMsgsObj);
    ...
    function duplicator_pro_s3_send_file_test()
    {
        ...
        try {
            ...
            if (DUP_PRO_S3_U::upload_file($s3_client, $bucket, $source_filepath, $storage_folder, $storage_class, $ACL_full_control)) {
                $json['success'] = true;
