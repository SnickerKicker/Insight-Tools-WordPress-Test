...
?>
    ...
    <table id="provider-<?php echo esc_attr(DUP_PRO_Storage_Types::S3); ?>" class="form-table provider" >
        ...
        <tr valign="top">
            ...
            <td>
                <input type="checkbox" name="s3_ACL_full_control" id="s3_ACL_full_control" value="1" <?php checked($storage->s3_ACL_full_control, true); ?> />
                <label for="s3_ACL_full_control"><?php DUP_PRO_U::esc_html_e("Enable full control ACL"); ?> </label><br />
                <p class="description">
...
$alert11->title = 'FTP Connection Status';
$alert11->height = 185;
$alert11->message = DUP_PRO_U::__('<i class="fas fa-exclamation-triangle"></i> Send FTP file test failed! Be sure the full storage path exists. For additional help see the online '
...
$alert14->title = 'FTP Connection Status';
$alert14->width = 600;
$alert14->height = 520;
$alert14->showTextArea = true;
$alert14->textAreaRows = 15;
$alert14->textAreaCols = 100;
...
$alert18 = new DUP_PRO_UI_Dialog();
$alert18->title = 'SFTP Connection Status';
$alert18->height = 185;
$alert18->message = DUP_PRO_U::__('<i class="fas fa-exclamation-triangle"></i> Send SFTP file test failed! Be sure the full storage path exists.');
$alert18->initAlert();

$alert19 = new DUP_PRO_UI_Dialog();
$alert19->title = 'SFTP Connection Status';
$alert19->width = 600;
$alert19->height = 450;
$alert19->showTextArea = true;
$alert19->textAreaRows = 13;
$alert19->textAreaCols = 100;
$alert19->message = '';  // javascript inserted message
$alert19->initAlert();
?>
<script>
...
<?php $alert10->showAlert(); ?>
            ...
            $.ajax({
                ...
                success: function (respData) {
                    try {
                        ...
                        $("#<?php echo $alert11->getID(); ?>_message").html("<i class='fas fa-exclamation-triangle'></i> <?php DUP_PRO_U::esc_html_e('JSON parse failed for response data'); ?>");
                        console.log(respData);
                        return false;
                    }
                    ...
                    if (parsedData.success) {
                        if (parsedData.status_msgs.length==0) {
                            <?php $alert11->showAlert(); ?>
                            $("#<?php echo $alert11->getID(); ?>_message").html("<span style='color:green'><b><input type='checkbox' class='checkbox' checked disabled='disabled'>"+parsedData.message+"</b></span>");
                        } else {
                            <?php $alert14->showAlert(); ?>
                            $("#<?php echo $alert14->getID(); ?>-textarea").val(parsedData.status_msgs);
                            $("#<?php echo $alert14->getID(); ?>_message").html("<span style='color:green'><b><input type='checkbox' class='checkbox' checked disabled='disabled'>"+parsedData.message+"</b></span>");
                        }
                    } else {
                        if (parsedData.status_msgs.length==0) {
                            <?php $alert11->showAlert(); ?>
                            $("#<?php echo $alert11->getID(); ?>_message").html("<i class='fas fa-exclamation-triangle'></i> "+parsedData.message);
                        } else {
                            <?php $alert14->showAlert(); ?>
                            $("#<?php echo $alert14->getID(); ?>-textarea").val(parsedData.status_msgs);
                            $("#<?php echo $alert14->getID(); ?>_message").html("<i class='fas fa-exclamation-triangle'></i> "+parsedData.message);
                        }
                        console.log(parsedData);
                    }
                    $test_button.html('<i class="fas fa-cloud-upload-alt fa-sm"></i> <?php DUP_PRO_U::esc_html_e('Test FTP Connection'); ?>');
                ...
                error: function (parsedData) {
                    $test_button.html('<i class="fas fa-cloud-upload-alt fa-sm"></i> <?php DUP_PRO_U::esc_html_e('Test FTP Connection'); ?>');
                    <?php $alert11->showAlert(); ?>
                    $("#<?php echo $alert11->getID(); ?>_message").html("<i class='fas fa-exclamation-triangle'></i> <?php DUP_PRO_U::esc_html_e('AJAX error while testing FTP connection'); ?>");
                    console.log(parsedData);
                }
            });
        }
            ...
            $.ajax({
                ...
                success: function (respData) {
                    try {
                        ...
                        <?php $alert18->showAlert(); ?>
                        $("#<?php echo $alert18->getID(); ?>_message").html("<i class='fas fa-exclamation-triangle'></i> <?php DUP_PRO_U::esc_html_e('JSON parse failed for response data'); ?>");
                        console.log(respData);
                        return false;
                    }
                    ...
                    if (parsedData.success) {
                        if (parsedData.status_msgs.length==0) {
                            <?php $alert18->showAlert(); ?>
                            $("#<?php echo $alert18->getID(); ?>_message").html("<span style='color:green'><b><input type='checkbox' class='checkbox' checked disabled='disabled'>"+parsedData.message+"</b></span>");
                        } else {
                            <?php $alert19->showAlert(); ?>
                            $("#<?php echo $alert19->getID(); ?>-textarea").val(parsedData.status_msgs);
                            $("#<?php echo $alert19->getID(); ?>_message").html("<span style='color:green'><b><input type='checkbox' class='checkbox' checked disabled='disabled'>"+parsedData.message+"</b></span>");
                        }
                    } else {
                        if (parsedData.status_msgs.length==0) {
                            <?php $alert18->showAlert(); ?>
                            $("#<?php echo $alert18->getID(); ?>_message").html("<i class='fas fa-exclamation-triangle'></i> "+parsedData.message);
                        } else {
                            <?php $alert19->showAlert(); ?>
                            $("#<?php echo $alert19->getID(); ?>-textarea").val(parsedData.status_msgs);
                            $("#<?php echo $alert19->getID(); ?>_message").html("<i class='fas fa-exclamation-triangle'></i> "+parsedData.message);
                        }
                        console.log(parsedData);
                    }
                    $test_button.html('<i class="fas fa-cloud-upload-alt fa-sm"></i> <?php DUP_PRO_U::esc_html_e('Test SFTP Connection'); ?>');
                ...
                error: function (respData) {
                    $test_button.html('<i class="fas fa-cloud-upload-alt fa-sm"></i> <?php DUP_PRO_U::esc_html_e('Test SFTP Connection'); ?>');
                    <?php $alert18->showAlert(); ?>
                    $("#<?php echo $alert18->getID(); ?>_message").html("<i class='fas fa-exclamation-triangle'></i> <?php DUP_PRO_U::esc_html_e('AJAX error while testing FTP connection'); ?>");
                    console.log(respData);
                }
            });
        }
        ...
        DupPro.Storage.S3.SendFileTest = function () {
            ...
            var current_ACL_full_control = $('#s3_ACL_full_control').prop('checked') ? 1 : 0;
